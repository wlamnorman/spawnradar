...
async def run_igdb_parent_game_backfill(settings: Settings) -> int:
    """One-shot job: populate parent_game_id for cached IGDB games."""
    sync = IGDBSyncService(
        db_path=settings.db_path,
        client_id=settings.twitch_client_id,
        client_secret=settings.twitch_client_secret,
    )
    try:
        updated = await sync.backfill_parent_games()
        logger.info("IGDB parent-game backfill: %d rows updated", updated)
        return updated
    except Exception:
        logger.exception("IGDB parent-game backfill job failed")
        return 0
