...
def _count_selected_genres(
    *,
    igdb_genre_ids: list[int] | None,
    igdb_keyword_ids: list[str] | None,
) -> int:
    """Count customer-visible genres across IGDB genres and genre keywords."""
    from app.igdb.keyword_groups import IGDB_KEYWORD_GROUPS, IGDBKeywordBucket

    genre_keyword_count = 0
    if igdb_keyword_ids:
        bucket_lookup = {kw.canonical: kw.bucket for kw in IGDB_KEYWORD_GROUPS}
        for keyword_id in igdb_keyword_ids:
            if bucket_lookup.get(keyword_id) == IGDBKeywordBucket.GENRE:
                genre_keyword_count += 1

    return len(igdb_genre_ids or []) + genre_keyword_count
...
class CustomerGameService:
    ...
    def create_game(
        ...
        if (
            _count_selected_genres(
                igdb_genre_ids=genres,
                igdb_keyword_ids=keywords,
            )
            < 2
        ):
            raise ValueError("At least 2 genres are required.")
