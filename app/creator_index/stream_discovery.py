...
from app.creator_index.twitch_http import TwitchAppAuth, twitch_request_json
...
class TwitchStreamClient:
    """Minimal Twitch Helix HTTP client backed by shared Twitch app auth.
    ...
    async def auth_headers(
        ...
        *,
        ...
        return await self._auth.auth_headers(http, force_refresh=force_refresh)
    ...
    async def get_streams(
        ...
        *,
        ...
        async with httpx.AsyncClient(timeout=20) as http:
            headers = await self.auth_headers(http)

            async def refresh_headers() -> dict[str, str]:
                nonlocal headers
                headers = await self.auth_headers(http, force_refresh=True)
                return headers
        ...
        return parse_streams_page(body)
    ...
    async def get_channel_info(
        ...
        return parse_channels_page(body)
    ...
    async def search_categories(
        ...
        *,
        ...
        return parse_category_search_page(body)
    ...
    async def get_games(
        ...
        *,
        ...
        return parse_games_page(body)


def parse_streams_page(payload: object) -> TwitchStreamsPage:
...
def parse_channels_page(payload: object) -> TwitchChannelsPage:
    if not isinstance(payload, dict):
...
def parse_category_search_page(payload: object) -> TwitchCategorySearchPage:
    if not isinstance(payload, dict):
...
def parse_games_page(payload: object) -> TwitchGamesPage:
    if not isinstance(payload, dict):
...
def build_streams_query_params(
    *,
    user_ids: Sequence[str] = (),
    user_logins: Sequence[str] = (),
    game_ids: Sequence[str] = (),
...
def build_category_search_query_params(
    *,
    query: str,
    first: int = 20,
    after: str | None = None,
