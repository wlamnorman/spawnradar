...
from app.creator_index.twitch_bundle import (
    extract_panel_contacts as extract_panel_contacts,  # noqa: F401
)
from app.creator_index.twitch_bundle import (
    infer_account_type as infer_account_type,  # noqa: F401
)
...
class TwitchEnrichment:
    ...
    async def _auth_headers(
        ...
        *,
        ...
        return await self._auth.auth_headers(
            client,
    ...
    async def fetch_users(
        ...
        async def _run(
            ...
            async def refresh_headers() -> dict[str, str]:
                nonlocal h
                h = await self._auth_headers(c, force_refresh=True)
                return h
        ...
        async def _run(
            ...
            for chunk in chunks([bid for bid in broadcaster_ids if bid], 100):
                ...
                for item in as_list(body.get("data")):
                    if not isinstance(item, dict):
                        continue
                    channel_info = parse_channel_info_record(item)
        ...
        async def _run(
            ...
            for chunk in chunks([bid for bid in broadcaster_ids if bid], 100):
                ...
                for item in as_list(body.get("data")):
                    if not isinstance(item, dict):
                        continue
                    stream = parse_stream_record(item)
