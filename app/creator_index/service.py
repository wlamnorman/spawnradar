...
class CreatorIndexService:
    ...
    def _prioritise_games(
        ...
        Priority:
        1. Active customers first — games whose owner viewed matches
    ...
    def _get_recently_active_game_ids(
        ...
        Activity = match page view or game definition update.
        """
        if not game_ids:
            return set()
        ...
        with get_connection(self._db_path) as conn:
            rows = conn.execute(
                ...
                FROM metric_events
                WHERE metric_key = 'match_pages_viewed'
