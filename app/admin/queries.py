...
def get_dashboard_data(db_path: str) -> dict:
    ...
    with get_connection(db_path) as conn:
        ...
        # All games with match counts and last match view
        games = conn.execute(
            """
            SELECT
                cg.customer_game_id,
                ...
                (SELECT count(*)
                 ...
                 WHERE ps.customer_game_id = cg.customer_game_id) AS match_count,
                (SELECT max(me.occurred_at)
                 FROM metric_events me
                 WHERE me.customer_game_id = cg.customer_game_id
                   AND me.metric_key = 'match_pages_viewed') AS last_match_view
