...
def _steam_api_descriptions(data: dict[str, Any], key: str) -> list[str]:
    items = data.get(key)
    if not isinstance(items, list):
        return []
...
def _supported_languages(data: dict[str, Any]) -> list[str]:
    ...
    return _dedupe_preserve_order(languages)
...
class SteamStoreAdapter:
    ...
    def _build_source_data(
        ...
        *,
        ...
        platform_labels = _platform_labels(api_payload)
        api_genre_labels = _steam_api_descriptions(api_payload, "genres")
        api_category_labels = _steam_api_descriptions(
            api_payload, "categories"
        )
        raw_tags = _dedupe_preserve_order(
            _extract_store_tags(store_html) + _steam_api_tags(api_payload)
        )
