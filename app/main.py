...
from app.matches.models import MATCH_WORKFLOW_STATUS_LABELS
...
def _home_template_context(
    *, ownership: OwnershipContext, settings: Settings
) -> dict[str, str | object | None]:
    """Return the public landing page template context."""
...
def _how_it_works_template_context(
    *, ownership: OwnershipContext
) -> dict[str, object | None]:
    """Return the public how-it-works page template context."""
    workflow_descriptions = {
        "suggested": "Suggested creators you have not triaged yet.",
        "to_contact": "Creators you have shortlisted for outreach.",
        "contacted": "Creators you have already reached out to.",
        "replied": "Creators who have responded to your outreach.",
        "to_cover": "Creators who are in the follow-through stage before coverage.",
        "covered": "Creators who have posted coverage of the game.",
        "not_pursuing": "Creators you decided not to pursue further.",
    }
    workflow_statuses = [
        {
            "value": status,
            "label": MATCH_WORKFLOW_STATUS_LABELS[status],
            "description": workflow_descriptions[status],
        }
        for status in MATCH_WORKFLOW_STATUS_LABELS
    ]
    return {
        "user": ownership.actor,
...
def create_app() -> FastAPI:
    ...
    app = FastAPI(
        title="Spawnradar",
        description="Multi-game creator matching and outreach for indie developers.",
        version="0.1.0",
        lifespan=lifespan,
    )
    ...
    async def root(
        ...
        return templates.TemplateResponse(
            request,
            "marketing/home.html",
            _home_template_context(ownership=ownership, settings=settings),
        )
    ...
    async def root_head(
        ...
        """Serve homepage HEAD probes with the same headers as GET."""
        return templates.TemplateResponse(
            request,
            "marketing/home.html",
            _home_template_context(ownership=ownership, settings=settings),
    ...
    async def how_it_works(
        ...
        """Render the public how-it-works page."""
        return templates.TemplateResponse(
            request,
            "marketing/how_it_works.html",
            _how_it_works_template_context(ownership=ownership),
        )

    @app.head("/how-it-works")
    async def how_it_works_head(
        request: Request,
        ownership: OwnershipContext = Depends(get_ownership_context),
        templates: Jinja2Templates = Depends(get_templates),
    ):
        """Serve how-it-works HEAD probes with the same headers as GET."""
        return templates.TemplateResponse(
            request,
            "marketing/how_it_works.html",
            _how_it_works_template_context(ownership=ownership),
