...
from app.twitch.http import TwitchAppAuth
...
class IGDBClient:
    ...
    async def raw_post(self, url: str, body: str) -> list[dict]:
        """Low-level POST to an IGDB endpoint. Returns parsed JSON."""
        async with httpx.AsyncClient(timeout=30) as http:
            resp = await http.post(
                url,
                headers=await self._auth.auth_headers(http),
                content=body,
            )
    ...
    async def fetch_games_by_name(
        ...
        When a result has a parent_game (DLC/expansion), the parent is also
        fetched and appended so callers can cache it locally.
        """
        safe_name = name.replace('"', '\\"')
        ...
        async with httpx.AsyncClient(timeout=30) as http:
            ...
            resp.raise_for_status()
            games = [self._parse(item) for item in resp.json()]

        # Eagerly fetch parent games so they land in the local cache.
        seen_ids = {g.igdb_id for g in games}
        parent_ids_needed = {
            g.parent_game_id
            for g in games
            if g.parent_game_id and g.parent_game_id not in seen_ids
        }
        for parent_id in parent_ids_needed:
            parent = await self.fetch_game_by_id(parent_id)
            if parent:
                games.append(parent)
                seen_ids.add(parent.igdb_id)
    ...
    def _build_games_query(*, limit: int, offset: int) -> str:
        ...
        return (
            ...
            "platforms.id,platforms.name,first_release_date,"
            "parent_game; "
            f"where genres = {genre_filter} & parent_game = null; "
    ...
    def _parse(item: dict) -> IGDBGame:
        ...
        return IGDBGame(
            ...
            parent_game_id=(
                int(item["parent_game"]) if item.get("parent_game") else None
            ),
        )
