...
def test_game_limit_returns_false_after_paid_limit(
    game_service, billing_service, registered_user
):
    # Paid Indie subscriptions allow 3 games.
    ...
    # Should now be at the paid Indie limit (3 games).
...
def test_create_game_accepts_genre_keyword_toward_minimum_genres(
    game_service, registered_user
):
    game = game_service.create_game(
        user_id=registered_user.user_id,
        name="Keyword Genre Count",
        summary="One IGDB genre plus one genre keyword should be enough.",
        description="One official genre and one mapped keyword genre.",
        website_url=None,
        igdb_genre_ids=[12],
        igdb_keyword_ids=["roguelike"],
    )

    assert game.igdb_genre_ids == [12]
    assert game.igdb_keyword_ids == ["roguelike"]


def test_update_game_accepts_genre_keyword_toward_minimum_genres(
    game_service, sample_game, registered_user
):
    updated = game_service.update_game(
        customer_game_id=sample_game.customer_game_id,
        user_id=registered_user.user_id,
        name="Updated Keyword Genre Count",
        summary="One IGDB genre plus one genre keyword should be enough.",
        description="One official genre and one mapped keyword genre.",
        website_url=None,
        igdb_genre_ids=[12],
    ...
    assert updated.igdb_genre_ids == [12]
    assert updated.igdb_keyword_ids == ["roguelike"]
