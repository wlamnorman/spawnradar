...
class TestCheckoutSuccessPage:
    def test_renders_polling_page_for_pre_subscription_user(
        self, monkeypatch, tmp_path
    ):
        client, db_path = _make_client(monkeypatch, tmp_path)
        with client:
...
class TestBillingManagementPage:
    def test_free_user_is_redirected_to_pricing(self, monkeypatch, tmp_path):
        client, db_path = _make_client(monkeypatch, tmp_path)
        with client:
            _register_and_verify(client, db_path)
...
class TestBillingNav:
    def test_free_user_sees_pricing_in_nav(self, monkeypatch, tmp_path):
        ...
        assert 'href="/pricing"' in response.text or response.status_code in (
            200,
        )
...
class TestFullActivationFlow:
    def test_checkout_activates_via_webhook_and_status_endpoint_drives_redirect(
        self, monkeypatch, tmp_path
    ):
        """Simulates the complete path: register → checkout → webhook → polling → active.
    ...
    def test_checkout_to_canceled_full_lifecycle(self, monkeypatch, tmp_path):
        """Pre-subscription → active → canceled → loses access after period end."""
        client, db_path = _make_client(monkeypatch, tmp_path)
        with client:
            _register_and_verify(client, db_path)
            user_id = _get_user_id(db_path, "user@example.com")
