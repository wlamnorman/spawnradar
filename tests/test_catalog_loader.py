...
class TestLoadCatalogGame:
    def test_basic_fields(self, tmp_path: Path):
        ...
        assert game.user_id == CATALOG_USER_ID
...
class TestLoadRealAppCatalogDefinitions:
    def test_loads_app_catalog_definitions(self):
        ...
        assert len(games) >= 35
        assert "Catalog: Survival Crafting" in names
        assert "Catalog: Competitive FPS" in names
        assert "Catalog: MMO RPG" in names
        assert "Catalog: Creature Collector RPG" in names
        assert "Catalog: Bullet Heaven Survivors" in names
        assert "Catalog: Immersive Sim" in names
        assert "Catalog: Visual Novel Narrative" in names
        assert "Catalog: Rhythm Action" in names
        assert "Catalog: Mech Combat" in names
        assert "Catalog: Open-World Sandbox RPG" in names
        assert "Catalog: Sports Management" in names
        assert "Catalog: Cozy Decorating / Home Design" in names
        assert "Catalog: Naval / Space Vehicle Sim" in names
