...
def _expire_inactive_subscription(db_path: str, email: str) -> str:
    """Simulate a user with no active subscription (free/expired).

    This helper cancels the subscription and sets its period end in the past
    so the user loses access.
    """
    with get_connection(db_path) as conn:
        row = conn.execute(
            """
    ...
    *,
    ...
    with get_connection(db_path) as conn:
        ...
        conn.execute(
            ...
            VALUES (?, ?, ?, ?)
            """,
            (999, "genre", "Tactical", 24),
        )
        for idx in range(count):
            account_id = f"match-{idx:03d}"
            handle = f"match{idx:03d}"
            conn.execute(
                """
...
class TestGameRoutes:
    ...
    def test_matches_page_shows_bucketed_curated_tags(
        self, monkeypatch, tmp_path
    ):
        db_path = str(tmp_path / "test.sqlite3")
        with _make_client(monkeypatch, tmp_path) as client:
            _register_and_login(
                client, "matches-buckets@example.com", "testpass"
            )
            ...
            page = client.get(f"/games/{game_row['slug']}/matches")
    ...
    def test_matches_page_highlights_played_similar_games(
        self, monkeypatch, tmp_path
    ):
        db_path = str(tmp_path / "test.sqlite3")
        with _make_client(monkeypatch, tmp_path) as client:
            _register_and_login(
                client, "matches-similar-highlight@example.com", "testpass"
            )
            _activate_paid_subscription(
                db_path, "matches-similar-highlight@example.com"
            )
            _post_form(
                client,
                get_path="/games/setup",
            ...
            with get_connection(db_path) as conn:
                ...
                conn.execute(
                    ...
                    (
                        "similar-highlight",
                        "twitch",
                        "ext-similar-highlight",
                        "similarhighlight",
                        "Similar Highlight",
                        "https://twitch.example.com/similarhighlight",
                    ),
                )
                conn.execute(
                    """
        ...
        assert page.status_code == 200
        assert "game-icon-wrap--similar" in page.text
        assert 'data-tooltip="Similar game: Anchor Match"' in page.text

    def test_matches_page_does_not_highlight_llm_only_similar_games(
        self, monkeypatch, tmp_path
    ):
        db_path = str(tmp_path / "test.sqlite3")
        with _make_client(monkeypatch, tmp_path) as client:
            _register_and_login(
                client, "matches-llm-similar@example.com", "testpass"
            )
            _activate_paid_subscription(
                db_path, "matches-llm-similar@example.com"
            )
            _post_form(
                client,
                get_path="/games/setup",
                post_path="/games/setup",
                data={
                    "name": "LLM Similar Highlight Game",
                    "summary": "Tactical RPG",
                    "description": "Tactical RPG",
                    "igdb_genre_ids": ["12", "24"],
                    "website_url": "",
            ...
            with get_connection(db_path) as conn:
                ...
                conn.execute(
                    ...
                    WHERE customer_game_id = ?
                    """,
                    ('["Anchor Match"]', game_row["customer_game_id"]),
                )
                conn.execute(
                    """
                    INSERT INTO source_accounts (
                    ...
                    """,
                    (
                        "llm-similar-highlight",
                        "twitch",
                        "ext-llm-similar-highlight",
                        "llmsimilarhighlight",
                        "LLM Similar Highlight",
                        "https://twitch.example.com/llmsimilarhighlight",
                    ),
                )
                conn.execute(
                    """
                ...
                conn.executemany(
                    ...
                    """,
                    [
                        (998, "genre", "Role-playing (RPG)", 12),
                        (998, "genre", "Tactical", 24),
                    ],
                )
                conn.execute(
                    """
        ...
        assert page.status_code == 200
        assert "Anchor Match" in page.text
        assert "game-icon-wrap--similar" not in page.text
        assert 'data-tooltip="Similar game: Anchor Match"' not in page.text

    def test_matches_page_uses_true_reach_filter_max(
        self, monkeypatch, tmp_path
    ):
        db_path = str(tmp_path / "test.sqlite3")
        with _make_client(monkeypatch, tmp_path) as client:
            _register_and_login(
                client, "matches-reach-max@example.com", "testpass"
            )
            _activate_paid_subscription(
                db_path, "matches-reach-max@example.com"
            )
            _post_form(
                client,
                get_path="/games/setup",
            ...
            game_slug = _seed_ranked_matches(
                db_path,
                count=3,
                customer_game_name="Reach Max Match Game",
            ...
            response = client.get(f"/games/{game_slug}/matches")
        ...
        assert 'data-range-floor="50"' in response.text

    def test_matches_page_allows_lower_reach_than_default(
        self, monkeypatch, tmp_path
    ):
        db_path = str(tmp_path / "test.sqlite3")
        with _make_client(monkeypatch, tmp_path) as client:
            _register_and_login(
                client, "matches-lower-reach@example.com", "testpass"
            )
            _activate_paid_subscription(
                db_path, "matches-lower-reach@example.com"
            )
            _post_form(
                client,
                get_path="/games/setup",
            ...
            game_slug = _seed_ranked_matches(
                db_path,
                count=3,
                customer_game_name="Lower Reach Match Game",
                game_name="Lower Reach Match",
            )

            response = client.get(f"/games/{game_slug}/matches?min_reach=50")

        assert response.status_code == 200
        assert 'name="min_reach"' in response.text
        assert 'value="50"' in response.text

    def test_matches_page_expands_games_filter_max_when_reach_widens(
        self, monkeypatch, tmp_path
    ):
        db_path = str(tmp_path / "test.sqlite3")
        with _make_client(monkeypatch, tmp_path) as client:
            _register_and_login(
                client, "matches-games-max@example.com", "testpass"
            )
            _activate_paid_subscription(
                db_path, "matches-games-max@example.com"
            )
            _post_form(
                client,
                get_path="/games/setup",
            ...
            narrowed = client.get(
                f"/games/{slug}/matches?max_reach=100000&max_games=2"
            )
            widened = client.get(
                f"/games/{slug}/matches?max_reach=1000000&max_games=2"
            )
    ...
    def test_matches_page_excludes_creators_below_minimum_reach(
        self, monkeypatch, tmp_path
    ):
        db_path = str(tmp_path / "test.sqlite3")
        with _make_client(monkeypatch, tmp_path) as client:
            _register_and_login(
                client, "matches-min-reach@example.com", "testpass"
            )
            _activate_paid_subscription(
                db_path, "matches-min-reach@example.com"
            )
            _post_form(
                client,
                get_path="/games/setup",
            ...
            game_slug = _seed_ranked_matches(
                db_path,
                count=2,
                customer_game_name="Min Reach Match Game",
        ...
        assert response.status_code == 200
        assert 'data-status-tab="all"' in response.text
        assert 'matches-status-tab-count">1<' in response.text
        assert "Match 001" not in response.text

    def test_paid_matches_page_shows_all_features_with_pagination(
        self, monkeypatch, tmp_path
    ):
        db_path = str(tmp_path / "test.sqlite3")
        with _make_client(monkeypatch, tmp_path) as client:
            _register_and_login(client, "matches-paid@example.com", "testpass")
            _post_form(
                client,
                get_path="/games/setup",
                post_path="/games/setup",
                data={
                    "name": "Paid Match Game",
                    "summary": "Tactical RPG",
                    "description": "Tactical RPG",
                    "igdb_genre_ids": ["12", "24"],
                    "website_url": "",
            ...
            game_slug = _seed_ranked_matches(
                db_path,
                count=51,
                customer_game_name="Paid Match Game",
                game_name="Paid Match Match",
            )
        ...
        assert response.status_code == 200
        assert 'data-status-tab="all"' in response.text
        assert 'matches-status-tab-count">51<' in response.text
        # Paid users see all features unlocked; no legacy capped-results messaging.
        assert "Showing the top 50 creator matches" not in response.text
        assert 'title="Disabled' not in response.text
        # Pagination is available for paid users with >50 results
        assert "Next →" in response.text

    def test_limited_matches_page_shows_locked_pagination_preview(
        self, monkeypatch, tmp_path
    ):
        db_path = str(tmp_path / "test.sqlite3")
        with _make_client(monkeypatch, tmp_path) as client:
            _setup_anonymous_session(db_path, client)
            _create_game_for_user(client, "Limited Preview Match Game")
            game_slug = _seed_ranked_matches(
                db_path,
                count=51,
                customer_game_name="Limited Preview Match Game",
                game_name="Limited Preview Match",
            )

            response = client.get(f"/games/{game_slug}/matches")

        assert response.status_code == 200
        assert 'class="pagination pagination--locked"' in response.text
        assert (
            'data-tooltip="Showing a preview of creator matches. Subscribe '
            'to view full creator results."'
        ) in response.text
        assert 'class="upgrade-nudge"' not in response.text
        assert 'href="?page=2' not in response.text
        assert "Next →" in response.text

    def test_matches_page_shows_true_total_above_fetch_cap(
        self, monkeypatch, tmp_path
    ):
        db_path = str(tmp_path / "test.sqlite3")
        with _make_client(monkeypatch, tmp_path) as client:
            _register_and_login(
                client, "matches-true-total@example.com", "testpass"
            )
            _post_form(
                client,
                get_path="/games/setup",
            ...
            _activate_paid_subscription(
                db_path, "matches-true-total@example.com"
            )
            game_slug = _seed_ranked_matches(
                db_path,
                count=1005,
                customer_game_name="True Total Match Game",
        ...
        assert response.status_code == 200
        assert 'data-status-tab="all"' in response.text
        assert 'matches-status-tab-count">1005<' in response.text

    def test_matches_page_page_one_shows_single_ellipsis(
        self, monkeypatch, tmp_path
    ):
        db_path = str(tmp_path / "test.sqlite3")
        with _make_client(monkeypatch, tmp_path) as client:
            _register_and_login(
                client, "matches-single-ellipsis@example.com", "testpass"
            )
            _post_form(
                client,
                get_path="/games/setup",
                post_path="/games/setup",
                data={
                    "name": "Single Ellipsis Match Game",
                    "summary": "Tactical RPG",
                    "description": "Tactical RPG",
                    "igdb_genre_ids": ["12", "24"],
                    "website_url": "",
            ...
            _activate_paid_subscription(
                db_path, "matches-single-ellipsis@example.com"
            )
            game_slug = _seed_ranked_matches(
                db_path,
                count=1005,
                customer_game_name="Single Ellipsis Match Game",
                game_name="Single Ellipsis Match",
            )

            response = client.get(f"/games/{game_slug}/matches")

        assert response.status_code == 200
        assert response.text.count("style=\"align-self: center\">…</span>") == 1

    def test_paid_matches_page_two_is_accessible(self, monkeypatch, tmp_path):
        db_path = str(tmp_path / "test.sqlite3")
        with _make_client(monkeypatch, tmp_path) as client:
            _register_and_login(
                client, "matches-paid-redirect@example.com", "testpass"
            )
            _post_form(
                client,
                get_path="/games/setup",
            ...
            response = client.get(
                f"/games/{game_slug}/matches?page=2",
                follow_redirects=False,
            )
    ...
    def test_paid_matches_workflow_endpoint_is_allowed(
        self, monkeypatch, tmp_path
    ):
        db_path = str(tmp_path / "test.sqlite3")
        with _make_client(monkeypatch, tmp_path) as client:
            _register_and_login(
                client, "matches-paid-workflow@example.com", "testpass"
            )
            _post_form(
                client,
                get_path="/games/setup",
            ...
            response = _post_json(
                client,
                get_path=f"/games/{game_slug}/matches",
                post_path=f"/games/{game_slug}/matches/match-000/workflow",
                json_body={
                    "status": "contacted",
    ...
    def test_comped_user_has_full_matches_workflow_access(
        self, monkeypatch, tmp_path
    ):
        db_path = str(tmp_path / "test.sqlite3")
        with _make_client(monkeypatch, tmp_path) as client:
            _register_and_login(
                client, "matches-comped@example.com", "testpass"
            )
            _grant_comped_access(db_path, "matches-comped@example.com")
            _post_form(
                client,
                get_path="/games/setup",
                post_path="/games/setup",
            ...
            page = client.get(f"/games/{game_slug}/matches")
            response = _post_json(
                client,
                get_path=f"/games/{game_slug}/matches",
                post_path=f"/games/{game_slug}/matches/match-000/workflow",
        ...
        assert page.status_code == 200
        assert 'data-status-tab="to_contact"' in page.text
        assert 'data-status-tab="contacted"' in page.text
        assert 'class="match-workflow-menu"' in page.text
        assert response.status_code == 200
        assert response.json()["status"] == "contacted"

    def test_paid_user_can_access_second_matches_page(
        self, monkeypatch, tmp_path
    ):
        db_path = str(tmp_path / "test.sqlite3")
        with _make_client(monkeypatch, tmp_path) as client:
            ...
            _activate_paid_subscription(db_path, "matches-paid@example.com")
            game_slug = _seed_ranked_matches(
                db_path,
                count=21,
                customer_game_name="Paid Match Game",
            ...
            response = client.get(f"/games/{game_slug}/matches?page=2")

        assert response.status_code == 200
        assert "Showing a preview of creator matches." not in response.text
        assert 'data-status-tab="all"' in response.text
        assert 'matches-status-tab-count">21<' in response.text
        assert "Next →" not in response.text
        assert "← Previous" in response.text
        assert "data-range-filter-form" in response.text

    def test_matches_page_shows_status_tabs_and_hides_not_pursuing_by_default(
        self, monkeypatch, tmp_path
    ):
        db_path = str(tmp_path / "test.sqlite3")
        with _make_client(monkeypatch, tmp_path) as client:
            _register_and_login(
                client, "matches-status-tabs@example.com", "testpass"
            )
            _activate_paid_subscription(
                db_path, "matches-status-tabs@example.com"
            )
            _post_form(
                client,
                get_path="/games/setup",
        ...
        assert 'data-status-tab="to_contact"' in response.text
        assert 'data-status-tab="contacted"' in response.text
        assert 'data-status-tab="not_pursuing"' in response.text
        assert "data-match-status-summary" in response.text
        assert "Match 000" in response.text
        assert "Match 001" in response.text
        assert "Match 002" not in response.text

    def test_status_filtered_matches_page_shows_only_that_status(
        self, monkeypatch, tmp_path
    ):
        db_path = str(tmp_path / "test.sqlite3")
        with _make_client(monkeypatch, tmp_path) as client:
            _register_and_login(
                client, "matches-status-filter@example.com", "testpass"
            )
            _activate_paid_subscription(
                db_path, "matches-status-filter@example.com"
            )
            _post_form(
                client,
                get_path="/games/setup",
            ...
            response = client.get(
                f"/games/{game_slug}/matches?status=to_contact"
            )

        assert response.status_code == 200
        assert "Match 001" in response.text
        assert "Match 000" not in response.text
        assert "Match 002" not in response.text

    def test_update_match_workflow_endpoint_persists_status_and_notes(
        self, monkeypatch, tmp_path
    ):
        db_path = str(tmp_path / "test.sqlite3")
        with _make_client(monkeypatch, tmp_path) as client:
            _register_and_login(
                client, "matches-workflow-update@example.com", "testpass"
            )
            _activate_paid_subscription(
                db_path, "matches-workflow-update@example.com"
            )
            _post_form(
                client,
                get_path="/games/setup",
        ...
        assert payload["status"] == "to_contact"
        assert payload["status_label"] == "To Contact"
        assert payload["has_notes"] is True
        assert payload["status_counts"]["to_contact"] == 1
        assert payload["status_counts"]["all"] == 2
        with get_connection(db_path) as conn:
            row = conn.execute(
                """
                SELECT ps.status, ps.notes
                FROM match_statuses ps
                JOIN customer_games cg
                    ON cg.customer_game_id = ps.customer_game_id
                WHERE cg.name = ? AND ps.account_id = ?
                """,
                ("Workflow Update Game", "match-000"),
            ).fetchone()
        assert row is not None
        assert row["status"] == "to_contact"
        assert row["notes"] == "Ready for outreach"

    def test_update_match_workflow_endpoint_returns_filtered_count_for_refill(
        self, monkeypatch, tmp_path
    ):
        db_path = str(tmp_path / "test.sqlite3")
        with _make_client(monkeypatch, tmp_path) as client:
            _register_and_login(
                client, "matches-workflow-refill@example.com", "testpass"
            )
            _activate_paid_subscription(
                db_path, "matches-workflow-refill@example.com"
            )
            _post_form(
                client,
                get_path="/games/setup",
                post_path="/games/setup",
                data={
                    "name": "Workflow Refill Route Game",
                    "summary": "Tactical RPG",
                    "description": "Tactical RPG",
                    "igdb_genre_ids": ["12", "24"],
                    "website_url": "",
            ...
            game_slug = _seed_ranked_matches(
                db_path,
                count=21,
                customer_game_name="Workflow Refill Route Game",
                game_name="Workflow Refill Route Match",
            )

            response = _post_json(
                client,
                get_path=f"/games/{game_slug}/matches?status=suggested",
                post_path=f"/games/{game_slug}/matches/match-020/workflow",
                json_body={
                    "status": "to_contact",
                    "notes": "Moved from New shortlist",
                    "active_status": "suggested",
                    "min_reach": 50,
                },
                follow_redirects=False,
                headers={"accept": "application/json"},
        ...
        assert payload["current_total_count"] == 20
        assert payload["status_counts"]["suggested"] == 20
        assert payload["status_counts"]["to_contact"] == 1
        assert payload["status_counts"]["all"] == 21
...
class TestBlogRoutes:
    ...
    def test_blog_index_head_returns_200(self, monkeypatch, tmp_path):
        with _make_client(monkeypatch, tmp_path) as client:
            resp = client.head("/blog")
        assert resp.status_code == 200
    ...
    def test_blog_post_head_returns_200(self, monkeypatch, tmp_path):
        with _make_client(monkeypatch, tmp_path) as client:
            resp = client.head(
                "/blog/indie-developer-creator-outreach-checklist"
            )
...
class TestSEORoutes:
    ...
    def test_robots_txt_does_not_include_explicit_allow_root(
        self, monkeypatch, tmp_path
    ):
        with _make_client(monkeypatch, tmp_path) as client:
            resp = client.get("/robots.txt")
    ...
    def test_robots_txt_does_not_disallow_games(self, monkeypatch, tmp_path):
        with _make_client(monkeypatch, tmp_path) as client:
            resp = client.get("/robots.txt")
        assert "Disallow: /games" not in resp.text
    ...
    def test_head_homepage_returns_200(self, monkeypatch, tmp_path):
        with _make_client(monkeypatch, tmp_path) as client:
            resp = client.head("/")
        assert resp.status_code == 200
        assert resp.headers["content-type"].startswith("text/html")

    def test_head_robots_txt_returns_200(self, monkeypatch, tmp_path):
        with _make_client(monkeypatch, tmp_path) as client:
            resp = client.head("/robots.txt")
        assert resp.status_code == 200
    ...
    def test_head_sitemap_returns_200(self, monkeypatch, tmp_path):
        with _make_client(monkeypatch, tmp_path) as client:
            resp = client.head("/sitemap.xml")
        assert resp.status_code == 200
    ...
    def test_sitemap_includes_how_it_works_page(self, monkeypatch, tmp_path):
        with _make_client(monkeypatch, tmp_path) as client:
            resp = client.get("/sitemap.xml")
        assert "/how-it-works</loc>" in resp.text

    def test_how_it_works_page_includes_matches_screenshot(
        self, monkeypatch, tmp_path
    ):
        with _make_client(monkeypatch, tmp_path) as client:
            resp = client.get("/how-it-works")
        assert resp.status_code == 200
        assert "matches-example.png" in resp.text
        assert "A ranked match list combines fit signals" in resp.text
...
class TestAuthRoutes:
    ...
    def test_google_login_uses_canonical_base_url_for_callback(
        ...
        app.state.oauth._clients["google"] = object()
...
class TestBillingRoutes:
    ...
    def test_billing_root_uses_pricing_style_price_breakdown(
        self, monkeypatch, tmp_path
    ):
        db_path = str(tmp_path / "test.sqlite3")
        ...
        with _make_client(monkeypatch, tmp_path) as client:
            _post_form(
                ...
                post_path="/auth/register",
                data={
                    "email": "billing-style@example.com",
                    "password": "testpass",
                },
            )
            _verify_user_email(db_path, "billing-style@example.com")
            _post_form(
                client,
                get_path="/auth/login",
                post_path="/auth/login",
            ...
            _grant_subscription(db_path, "billing-style@example.com")

            resp = client.get("/billing")

        assert resp.status_code == 200
        assert 'class="plan-price-period"' in resp.text
        assert 'class="plan-price-note">+ applicable tax</div>' in resp.text
...
class TestAccessGate:
    def test_inactive_subscription_games_page_accessible(
        self, monkeypatch, tmp_path
    ):
        db = str(tmp_path / "test.sqlite3")
        with _make_client(monkeypatch, tmp_path) as client:
            _register_and_login(client, "expired@example.com", "testpass")
            _expire_inactive_subscription(db, "expired@example.com")
    ...
    def test_inactive_subscription_setup_page_accessible(
        self, monkeypatch, tmp_path
    ):
        db = str(tmp_path / "test.sqlite3")
        with _make_client(monkeypatch, tmp_path) as client:
            _register_and_login(client, "newexpired@example.com", "testpass")
            _expire_inactive_subscription(db, "newexpired@example.com")

            resp = client.get("/games/setup", follow_redirects=False)

        assert resp.status_code == 200

    def test_inactive_subscription_game_edit_page_accessible(
        self, monkeypatch, tmp_path
    ):
        db = str(tmp_path / "test.sqlite3")
        with _make_client(monkeypatch, tmp_path) as client:
            ...
            _expire_inactive_subscription(db, "setupexpired@example.com")
...
class TestAnonymousFlows:
    ...
    # ------------------------------------------------------------------
    # 4. Anonymous user can view matches page 1
    # ------------------------------------------------------------------
    def test_anonymous_user_can_view_matches_page_1(
        self, monkeypatch, tmp_path
    ) -> None:
        db_path = str(tmp_path / "test.sqlite3")
        with _make_client(monkeypatch, tmp_path) as client:
            _setup_anonymous_session(db_path, client)
            _create_game_for_user(client, "Anon Matches Game")
            slug = _get_game_slug(db_path, "Anon Matches Game")

            response = client.get(f"/games/{slug}/matches")

        assert response.status_code == 200
        assert "Sign up" in response.text

    # ------------------------------------------------------------------
    # 5. Anonymous user cannot access page 2 of matches
    # ------------------------------------------------------------------
    def test_anonymous_user_cannot_access_page_2_matches(
        self, monkeypatch, tmp_path
    ) -> None:
        db_path = str(tmp_path / "test.sqlite3")
        with _make_client(monkeypatch, tmp_path) as client:
            ...
            response = client.get(
                f"/games/{slug}/matches?page=2", follow_redirects=False
            )

        assert response.status_code == 303
        assert response.headers["location"] == f"/games/{slug}/matches"
    ...
    def test_anonymous_user_cannot_use_filters(
        ...
        with _make_client(monkeypatch, tmp_path) as client:
            ...
            # min_games=2 reliably adds to filter_params (non-default value)
            response = client.get(
                f"/games/{slug}/matches?min_games=2",
                follow_redirects=False,
            )
    ...
    def test_anonymous_user_cannot_use_workflow(
        ...
        with _make_client(monkeypatch, tmp_path) as client:
            ...
            response = client.post(
                f"/games/{slug}/matches/fake-account-id/workflow",
    ...
    def test_pricing_page_shows_free_tier(self, monkeypatch, tmp_path) -> None:
        ...
        assert "Simple pricing for creator outreach." in response.text
    ...
    # ------------------------------------------------------------------
    # 15. Match page renders filter form for free users (interactive gate)
    # ------------------------------------------------------------------
    def test_free_user_sees_interactive_filter_form(
        ...
        assert "data-range-filter-form" in response.text
        assert "matches-filter-menu" in response.text
    ...
    def test_free_user_workflow_post_returns_403(
        ...
        with _make_client(monkeypatch, tmp_path) as client:
            ...
            # Attempt to update match workflow (JSON API)
            response = _post_json(
                client,
                get_path=f"/games/{slug}/matches",
                post_path=f"/games/{slug}/matches/fake-account/workflow",
    ...
    def test_server_strips_filters_for_free_user(
        ...
        with _make_client(monkeypatch, tmp_path) as client:
            ...
            response = client.get(
                f"/games/{slug}/matches?min_reach=5000&status=contacted",
                follow_redirects=False,
            )
    ...
    # ------------------------------------------------------------------
    # 21. Paid user matches page still computes filter ranges
    # ------------------------------------------------------------------
    def test_paid_user_matches_has_filter_form_with_ranges(
        self, monkeypatch, tmp_path
    ) -> None:
        db_path = str(tmp_path / "test.sqlite3")
        with _make_client(monkeypatch, tmp_path) as client:
