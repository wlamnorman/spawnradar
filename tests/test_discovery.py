...
from app.creator_index.discovery import (
    EnrichedCreator,
    _resolve_similar_games,
    _run_keyword_queries,
...
async def test_resolve_similar_games_picks_exact_match():
    ...
    result = await _resolve_similar_games(
...
async def test_resolve_similar_games_skips_not_found():
    ...
    result = await _resolve_similar_games(["NonexistentGame"], mock_client)
    assert result == []
...
async def test_keyword_queries_progressive_fallback():
    ...
    result = await _run_keyword_queries(game, mock_client, set())
...
async def test_keyword_queries_skips_existing_ids():
    ...
    result = await _run_keyword_queries(game, mock_client, existing)
