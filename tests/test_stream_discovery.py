...
from app.creator_index.stream_discovery import (
    ...
    TwitchGamesPage,
    TwitchPagination,
    _build_category_search_query_params,
    _build_games_query_params,
    _build_streams_query_params,
    _parse_category_search_page,
    _parse_games_page,
)
...
def test_build_streams_query_params_rejects_before_and_after_together():
    with pytest.raises(ValueError, match="before and after"):
        _build_streams_query_params(before="a", after="b")
...
def test_parse_category_search_page():
    page = _parse_category_search_page(
        {
            "data": [
                {
                    "id": "509658",
