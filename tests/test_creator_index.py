...
def test_parse_clip_record_extracts_game_id_and_view_count():
    from app.creator_index.enrichment import parse_clip_record
...
def test_parse_clip_record_returns_none_for_missing_id():
    ...
    assert parse_clip_record({"title": "no id"}) is None


def test_parse_clip_record_returns_none_for_missing_game_id():
    from app.creator_index.enrichment import parse_clip_record

    assert parse_clip_record({"id": "c1", "title": "t"}) is None
...
def test_async_persisted_creator_becomes_rankable_match(db_path):
    ...
    creator = EnrichedCreator(
        bundle=AccountSeedBundle(
            account=SourceAccountSeed(
                external_id="tw-match",
                handle_current="match-tv",
                display_name_current="Match TV",
                canonical_url="https://www.twitch.tv/match-tv",
            ),
            platform_profile=TwitchProfileSeed(
                broadcaster_id="tw-match",
                login="match-tv",
                display_name="Match TV",
                description=None,
    ...
    matches, total, _, _, _ = MatchRankingService(db_path).rank_matches(game)

    assert len(matches) == 1
    assert matches[0].profile.handle == "match-tv"
...
def test_extract_panel_contacts_finds_email_in_description():
    from app.creator_index.enrichment import extract_panel_contacts
...
def test_extract_panel_contacts_handles_empty_panels():
    ...
    assert extract_panel_contacts([], "testuser", set(), set()) == []
