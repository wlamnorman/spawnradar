...
class TestGetDashboardData:
    ...
    def test_user_with_no_games(
        self, db_path, auth_service, billing_service, sub_repo
    ):
        import uuid
    ...
    def test_user_with_game(
        ...
        assert g["last_match_view"] is None
...
def _register_and_login(client, email, password):
    ...
    client.post(
        "/auth/register",
        data={
            "csrf_token": _csrf("/auth/register"),
            "email": email,
            "password": password,
        },
    )
...
class TestAdminRouteAccessControl:
    ...
    def test_admin_user_with_correct_key_gets_200(self, admin_client):
        ...
        assert (
            "admin@test.com" not in resp.text
        )  # admin excluded from workspace list
