...
def test_root_renders_public_landing_page(monkeypatch, tmp_path):
    ...
    assert "Creator outreach for indie game developers" in text
    assert (
        "SpawnRadar helps indie game developers find creators (streamers, "
        "YouTubers and more) whose content and audiences fit their game for "
        "launch and ongoing marketing, and manage outreach in one place."
        in text
    )
    ...
    assert (
        "active creators with analyzed game interests and contact info" in text
    )
    assert "We surface likely creator fits" in text
...
def test_how_it_works_renders_public_explainer(monkeypatch, tmp_path):
    with _make_client(monkeypatch, tmp_path) as client:
        response = client.get("/how-it-works")

    text = " ".join(response.text.split())
    assert response.status_code == 200
    assert "SpawnRadar | How It Works" in text
    assert "<h1>How it works</h1>" in response.text
    assert "What a match means" in text
    assert "A strong match does not mean" in text
    assert "What the workflow statuses mean" in text
    assert "To Contact" in text
    assert "Do you guarantee coverage?" in text
    assert 'class="how-it-works-page-actions"' not in response.text


def test_pricing_renders_free_and_paid_comparison(monkeypatch, tmp_path):
    with _make_client(monkeypatch, tmp_path) as client:
        response = client.get("/pricing")

    text = " ".join(response.text.split())
    assert response.status_code == 200
    assert "Simple pricing for creator outreach." in text
    assert "Free" in text
    assert "Paid" in text
    assert "subscribe" in text.lower()
    assert "Studio" not in text
    assert "Questions before subscribing?" in text
    assert "active game" in text
    assert "Preview creator matches" in text
    assert "No payment method required" in text
    assert "No expiry" in text
    assert "Up to" in text
    assert "active games" in text
    assert "Full creator results" in text
    assert "Advanced creator filtering" in text
    assert "Creator relations workflow" in text
