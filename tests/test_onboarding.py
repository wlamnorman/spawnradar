...
def _grant_subscription(db_path: str, email: str) -> None:
    ...
    repo.create(str(uuid.uuid4()), row["user_id"], Tier.INDIE)
...
def test_empty_dashboard_shows_add_card(monkeypatch, tmp_path):
    db_path = str(tmp_path / "games-flow.sqlite3")
    with _make_client(monkeypatch, tmp_path) as client:
        _post_form(
            client,
        ...
        _grant_subscription(db_path, "blank@example.com")
        response = client.get("/games")

    assert response.status_code == 200
    # Paid users have 3 slots, all unlocked — all 3 have setup links
    assert response.text.count('href="/games/setup"') == 3
    # No legacy locked-slot copy for paid users
...
def test_dashboard_game_cards_include_run_discovery_action(
    ...
    with _make_client(monkeypatch, tmp_path) as client:
        ...
        _grant_subscription(db_path, "runner@example.com")
        _post_form(
            client,
    ...
    # Paid user: no legacy locked-slot copy, 2 remaining unlocked setup slots
    assert "Locked" not in response.text
    assert response.text.count('href="/games/setup"') == 2
...
def test_create_game_redirects_to_dashboard(monkeypatch, tmp_path):
    ...
    with _make_client(monkeypatch, tmp_path) as client:
        ...
        _grant_subscription(db_path, "flow@example.com")
...
def test_pricing_page_shows_free_and_paid_plans(monkeypatch, tmp_path):
    with _make_client(monkeypatch, tmp_path) as client:
        _post_form(
            client,
            get_path="/auth/register",
    ...
    assert "Preview creator matches" in response.text
    assert "No payment method required" in response.text
    assert "No expiry" in response.text
    assert "Full creator results" in response.text
...
def test_dev_login_creates_session_when_enabled(monkeypatch, tmp_path):
    ...
    assert "Sign out" in home_response.text
