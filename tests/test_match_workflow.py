"""Integration tests for the match CRM workflow system.
...
def _setup_game_with_creators(
    ...
    _insert_igdb_game(
        db_path, 800, "RPG Match", genre_tags=[(12, "RPG"), (24, "Tactical")]
    )
...
class TestMatchWorkflowRepository:
    """Test the match_statuses table operations."""

    def test_default_state_for_unknown_match(self, db_path):
        """Matches without a row are implicitly 'suggested'."""
    ...
    def test_upsert_creates_new_status(
        ...
        result = repo.upsert_match_workflow_state(
    ...
    def test_upsert_updates_existing_status(
        self, db_path, game_service, registered_user
    ):
        game, creator_ids = _setup_game_with_creators(
            db_path,
        ...
        repo.upsert_match_workflow_state(
            customer_game_id=game.customer_game_id,
            account_id=creator_ids[0],
            status="contacted",
            notes="",
    ...
    def test_upsert_back_to_suggested_deletes_row(
        self, db_path, game_service, registered_user
    ):
        """Setting status back to 'suggested' with no notes removes the row (sparse storage)."""
        game, creator_ids = _setup_game_with_creators(
            db_path,
            game_service,
            registered_user,
    ...
    def test_upsert_suggested_with_notes_keeps_row(
        self, db_path, game_service, registered_user
    ):
        """Status 'suggested' but with notes should keep the row."""
        game, creator_ids = _setup_game_with_creators(
            db_path,
            game_service,
            registered_user,
    ...
    def test_legacy_status_values_are_mapped_forward(
        self, db_path, game_service, registered_user
    ):
        """Legacy stored workflow values still read back as current statuses."""
        game, creator_ids = _setup_game_with_creators(
            db_path,
            game_service,
            registered_user,
        ...
        with get_connection(db_path) as conn:
            conn.execute(
                ...
                """,
                (game.customer_game_id, creator_ids[0]),
            )
            conn.execute(
                """
                INSERT INTO match_statuses (
                    customer_game_id, account_id, status, notes, updated_at
                ) VALUES (?, ?, 'access_shared', '', datetime('now'))
                """,
                (game.customer_game_id, creator_ids[1]),
            )
        ...
        assert states[creator_ids[0]].status == "suggested"
        assert states[creator_ids[1]].status == "to_cover"

    def test_initialize_database_migrates_legacy_status_rows(
        self, db_path, game_service, registered_user
    ):
        """Startup migration rewrites old stored status keys in SQLite."""
        game, creator_ids = _setup_game_with_creators(
            db_path,
            game_service,
            registered_user,
    ...
    def test_count_workflow_statuses(
        self, db_path, game_service, registered_user
    ):
        game, creator_ids = _setup_game_with_creators(
            db_path,
...
class TestMatchWorkflowService:
    """Test workflow operations through MatchRankingService."""

    def test_matches_include_workflow_state(
        self,
        db_path,
        game_service,
        registered_user,
    ):
        """Each match in the ranked list includes its workflow state."""
        game, creator_ids = _setup_game_with_creators(
            db_path,
            game_service,
            registered_user,
        ...
        matches, total, status_counts, _, _ = service.rank_matches(game)

        contacted = [p for p in matches if p.workflow.status == "contacted"]
        suggested = [
            p for p in matches if p.workflow.status == "suggested"
        ]
    ...
    def test_status_filter_all_excludes_not_pursuing(
        ...
        """The 'all' filter hides 'not_pursuing' matches."""
        game, creator_ids = _setup_game_with_creators(
            db_path,
            game_service,
            registered_user,
        ...
        service = MatchRankingService(db_path)
        matches, total, status_counts, _, _ = service.rank_matches(
            game,
            status_filter="all",
        )

        ids = [p.profile.account_id for p in matches]
    ...
    def test_status_filter_not_pursuing_shows_only_hidden(
        ...
        matches, total, status_counts, _, _ = service.rank_matches(
            game,
            status_filter="not_pursuing",
        )

        assert total == 1
        assert matches[0].profile.account_id == creator_ids[0]
    ...
    def test_status_filter_specific_status(
        ...
        """Filtering by a specific status shows only matching matches."""
        game, creator_ids = _setup_game_with_creators(
            db_path,
            game_service,
            registered_user,
        ...
        assert total == 2
        assert all(p.workflow.status == "contacted" for p in matches)

    def test_status_filter_to_contact_shows_only_shortlisted(
        self,
        db_path,
        game_service,
        registered_user,
    ):
        """Filtering by To Contact shows only shortlisted matches."""
        game, creator_ids = _setup_game_with_creators(
            db_path,
            game_service,
            registered_user,
        ...
        matches, total, status_counts, _, _ = service.rank_matches(
            game,
            status_filter="to_contact",
        )

        assert total == 1
        assert [p.profile.account_id for p in matches] == [creator_ids[0]]
    ...
    def test_status_counts_reflect_all_matches(
        self,
        db_path,
        game_service,
        registered_user,
    ):
        """Status counts should cover all matching matches, not just the current page."""
        game, creator_ids = _setup_game_with_creators(
            db_path,
            game_service,
            registered_user,
        ...
        repo.upsert_match_workflow_state(
            customer_game_id=game.customer_game_id,
            account_id=creator_ids[3],
            status="covered",
            notes="",
        )

        service = MatchRankingService(db_path)
        _matches, _total, status_counts, _, _ = service.rank_matches(game)
    ...
    def test_workflow_state_persists_across_queries(
        ...
        """Status set via repository is visible in subsequent rank_matches calls."""
        game, creator_ids = _setup_game_with_creators(
            db_path,
            game_service,
            registered_user,
        ...
        service = MatchRankingService(db_path)
        matches1, _, _, _, _ = service.rank_matches(game)
        assert matches1[0].workflow.status == "suggested"
        ...
        matches2, _, _, _, _ = service.rank_matches(game)
        assert matches2[0].workflow.status == "contacted"
    ...
    def test_workflow_full_lifecycle(
        ...
        """Walk a match through the full status lifecycle."""
        game, creator_ids = _setup_game_with_creators(
            db_path,
            game_service,
            registered_user,
        ...
        lifecycle: list[MatchWorkflowStatus] = [
            "to_contact",
            "contacted",
            "replied",
            "to_cover",
    ...
    def test_different_games_have_independent_statuses(
        ...
        states1 = repo.get_match_workflow_states(
            customer_game_id=game1.customer_game_id,
            account_ids=("shared-creator",),
        )
        states2 = repo.get_match_workflow_states(
            customer_game_id=game2.customer_game_id,
            account_ids=("shared-creator",),
        )
