...
def test_parse_user_returns_none_for_missing_fields():
    assert _parse_user({"id": "1", "login": "x"}) is None
    assert _parse_user({}) is None
...
def test_parse_clip_record_returns_none_for_missing_game_id():
    ...
    assert _parse_clip_record(raw) is None
...
def test_infer_account_type_detects_developer():
    assert _infer_account_type("indie gamedev here", None, []) == "developer"
    assert _infer_account_type(None, "devlog stream", []) == "developer"
    assert _infer_account_type(None, None, ["indiedev"]) == "developer"


def test_infer_account_type_defaults_to_creator():
    assert (
        _infer_account_type("Just a streamer", "Playing Fortnite", ["fps"])
        == "creator"
    )
    assert _infer_account_type(None, None, []) == "creator"
