...
@pytest.fixture
def auth_service_with_verification(user_repo, session_repo, email_verification_token_repo):
    return AuthService(user_repo, session_repo, email_verification_token_repo=email_verification_token_repo)


@pytest.fixture
def unverified_user(auth_service_with_verification):
    return auth_service_with_verification.register("unverified@example.com", "password123")


@pytest.fixture
def verified_user(auth_service_with_verification, user_repo):
    user = auth_service_with_verification.register("verified@example.com", "password123")
...
class TestEmailVerificationTokenRepository:
    def test_create_and_get_token(self, email_verification_token_repo, unverified_user):
        token_id = str(uuid.uuid4())
        expires_at = (datetime.now(UTC) + timedelta(hours=24)).isoformat()
        email_verification_token_repo.create(token_id, unverified_user.user_id, expires_at)
    ...
    def test_get_by_id_returns_none_for_unknown_token(self, email_verification_token_repo):
        assert email_verification_token_repo.get_by_id("nonexistent") is None

    def test_get_by_id_returns_none_for_used_token(self, email_verification_token_repo, unverified_user):
        token_id = str(uuid.uuid4())
        expires_at = (datetime.now(UTC) + timedelta(hours=24)).isoformat()
        email_verification_token_repo.create(token_id, unverified_user.user_id, expires_at)
    ...
    def test_get_by_id_returns_none_for_expired_token(self, db_path, unverified_user):
    ...
    def test_mark_used_sets_used_at(self, email_verification_token_repo, unverified_user):
        token_id = str(uuid.uuid4())
        expires_at = (datetime.now(UTC) + timedelta(hours=24)).isoformat()
        email_verification_token_repo.create(token_id, unverified_user.user_id, expires_at)
        email_verification_token_repo.mark_used(token_id)
    ...
    def test_token_deleted_when_user_deleted(self, db_path, unverified_user):
        ...
        with get_connection(db_path) as conn:
            conn.execute("DELETE FROM users WHERE user_id = ?", (unverified_user.user_id,))

        with get_connection(db_path) as conn:
            row = conn.execute(
                "SELECT * FROM email_verification_tokens WHERE token_id = ?", (token_id,)
            ).fetchone()
...
class TestUserRepositoryVerification:
    ...
    def test_mark_email_verified_is_idempotent(self, user_repo, unverified_user):
        user_repo.mark_email_verified(unverified_user.user_id)
        user_repo.mark_email_verified(unverified_user.user_id)
        updated = user_repo.get_by_id(unverified_user.user_id)
        assert updated.email_verified is True
...
class TestAuthServiceEmailVerification:
    ...
    def test_verify_email_returns_false_for_unknown_token(self, auth_service_with_verification):
        assert auth_service_with_verification.verify_email("bad-token") is False
    ...
    def test_send_verification_email_is_noop_without_token_repo(self, auth_service, unverified_user):
        mock_email = MagicMock(spec=EmailService)
        # Should not raise and should not call send
        auth_service.send_verification_email(unverified_user, mock_email, "https://example.com")
        mock_email.send.assert_not_called()

    def test_mark_google_user_verified(
        self, auth_service_with_verification, user_repo, unverified_user
    ):
        auth_service_with_verification.mark_google_user_verified(unverified_user.user_id)
        updated = user_repo.get_by_id(unverified_user.user_id)
        assert updated.email_verified is True
    ...
    def test_verification_link_uses_base_url(
        ...
        assert "https://myapp.example.com/auth/verify-email?token=" in sent[0].html
...
class TestRegistrationFlow:
    def test_register_redirects_to_verify_pending(self, monkeypatch, tmp_path):
        ...
        with client:
            ...
            response = client.post(
                "/auth/register",
                data={"email": "new@example.com", "password": "pass123", "csrf_token": token},
                follow_redirects=False,
            )
    ...
    def test_unverified_user_sees_verify_pending_page(self, monkeypatch, tmp_path):
        client, db_path = _make_http_client(monkeypatch, tmp_path)
        with client:
    ...
    def test_verified_user_redirected_away_from_verify_pending(self, monkeypatch, tmp_path):
        client, db_path = _make_http_client(monkeypatch, tmp_path)
        with client:
            _register(client, "done@example.com")
            with get_connection(db_path) as conn:
                conn.execute("UPDATE users SET email_verified = 1 WHERE email = 'done@example.com'")
...
class TestVerificationLink:
    def test_valid_token_verifies_user_and_redirects_to_games(self, monkeypatch, tmp_path):
        client, db_path = _make_http_client(monkeypatch, tmp_path)
        with client:
            _register(client, "click@example.com")
            token_id = _get_verification_token(db_path, "click@example.com")
            response = client.get(f"/auth/verify-email?token={token_id}", follow_redirects=False)
    ...
    def test_invalid_token_redirects_to_verify_pending_with_error(self, monkeypatch, tmp_path):
        client, db_path = _make_http_client(monkeypatch, tmp_path)
        with client:
            _register(client, "badtoken@example.com")
            response = client.get("/auth/verify-email?token=not-a-real-token", follow_redirects=False)
        assert response.status_code == 303
        assert "error=invalid" in response.headers["location"]

    def test_missing_token_param_redirects_with_error(self, monkeypatch, tmp_path):
        client, db_path = _make_http_client(monkeypatch, tmp_path)
        with client:
    ...
    def test_token_is_single_use(self, monkeypatch, tmp_path):
        ...
        with client:
            ...
            token_id = _get_verification_token(db_path, "singleuse@example.com")

            # First use succeeds
            r1 = client.get(f"/auth/verify-email?token={token_id}", follow_redirects=False)
            assert r1.headers["location"] == "/games"

            # Second use with the same token fails
            r2 = client.get(f"/auth/verify-email?token={token_id}", follow_redirects=False)
    ...
    def test_expired_token_is_rejected(self, monkeypatch, tmp_path):
        ...
        with client:
            ...
            real_token = _get_verification_token(db_path, "expired@example.com")
            ...
            assert _get_verification_token(db_path, "expired@example.com") is None

            response = client.get(
                f"/auth/verify-email?token={real_token}", follow_redirects=False
            )
        assert "error=invalid" in response.headers["location"]
        assert _is_email_verified(db_path, "expired@example.com") is False

    def test_error_message_shown_on_invalid_token_page(self, monkeypatch, tmp_path):
        client, db_path = _make_http_client(monkeypatch, tmp_path)
        with client:
            _register(client, "showerr@example.com")
            # Follow redirect to the pending page with ?error=invalid
            response = client.get("/auth/verify-email?token=bad", follow_redirects=True)
...
class TestResendVerification:
    def test_resend_creates_new_token(self, monkeypatch, tmp_path):
        ...
        with client:
            ...
            response = client.get("/auth/resend-verification", follow_redirects=False)
            assert response.status_code == 303
    ...
    def test_resend_redirects_already_verified_user_to_games(self, monkeypatch, tmp_path):
        client, db_path = _make_http_client(monkeypatch, tmp_path)
        with client:
            _register(client, "alreadydone@example.com")
            with get_connection(db_path) as conn:
                conn.execute("UPDATE users SET email_verified = 1 WHERE email = 'alreadydone@example.com'")
            response = client.get("/auth/resend-verification", follow_redirects=False)
        assert response.headers["location"] == "/games"
    ...
    def test_second_token_can_verify_user(self, monkeypatch, tmp_path):
        ...
        with client:
            ...
            r = client.get(f"/auth/verify-email?token={new_token}", follow_redirects=False)
...
class TestDevLogin:
    def test_dev_login_user_is_auto_verified(self, monkeypatch, tmp_path):
        ...
        for key in ("PADDLE_API_KEY", "PADDLE_CLIENT_SIDE_TOKEN", "PADDLE_WEBHOOK_SECRET",
                    "PADDLE_INDIE_PRICE_ID", "PADDLE_ENVIRONMENT",
    ...
    def test_dev_login_does_not_redirect_to_verify_pending(self, monkeypatch, tmp_path):
...
class TestAbuseResistance:
    def test_one_email_one_trial(self, monkeypatch, tmp_path):
        ...
        with client:
            ...
            response = client.post(
                "/auth/register",
                data={"email": "onetime@example.com", "password": "pass123", "csrf_token": token},
                follow_redirects=False,
            )
    ...
    def test_gmail_dot_trick_is_normalized(self, monkeypatch, tmp_path):
        ...
        with client:
            ...
            response = client.post(
                "/auth/register",
                data={"email": "m.e@gmail.com", "password": "pass123", "csrf_token": token},
                follow_redirects=False,
            )
        assert response.status_code in (200, 400)
        assert "already exists" in response.text.lower()
    ...
    def test_gmail_plus_tag_is_normalized(self, monkeypatch, tmp_path):
        ...
        with client:
            ...
            response = client.post(
                "/auth/register",
                data={"email": "me+trial@gmail.com", "password": "pass123", "csrf_token": token},
                follow_redirects=False,
            )
        assert response.status_code in (200, 400)
        assert "already exists" in response.text.lower()

    def test_unauthenticated_user_cannot_reach_verify_pending(self, monkeypatch, tmp_path):
        client, db_path = _make_http_client(monkeypatch, tmp_path)
        with client:
            response = client.get("/auth/verify-pending", follow_redirects=False)
    ...
    def test_unauthenticated_user_cannot_reach_resend(self, monkeypatch, tmp_path):
        client, db_path = _make_http_client(monkeypatch, tmp_path)
        with client:
            response = client.get("/auth/resend-verification", follow_redirects=False)
        assert response.status_code in (303, 307)
